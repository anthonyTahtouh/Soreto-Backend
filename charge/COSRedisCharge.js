      ...
      if(userList && userList.length > 0){
        ...
        for (let user of userList) {
          if(user && user.buyer_email){
            ...
            //charge_log(`----- Creating row ${user.buyer_email} at ${user.created_at}, ttl in days = ${diffDays}`);
            await securityService.orderFreshUser.setOrderFreshUserBatch(clientId, user.buyer_email, diffDays);
          }
        }
