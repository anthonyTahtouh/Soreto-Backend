export function up(knex) {
  ...
  SELECT users.email,
      ...
      reward_code.discount_type AS "discountType",
      orders._id AS "orderId",
      campaign_version._id AS "campaignVersionId",
      reward_code.code,
      reward_code.value_amount AS "valueAmount",
	    users._id AS "userId"
    FROM reverb."order" orders
      ...
      LEFT JOIN reverb.campaign_version campaign_versions ON campaign_versions.reward_pool_id = reward_pool._id
      LEFT JOIN reverb.reward_discount_code reward_code ON rewards._id = reward_code.reward_id
