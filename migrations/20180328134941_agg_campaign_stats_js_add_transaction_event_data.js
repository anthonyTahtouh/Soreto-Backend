  ...
  CREATE OR REPLACE VIEW reverb.agg_campaign_stats_js AS
    SELECT
      ...
      WHEN sum(COALESCE(social_post.shares, 0::bigint)) <> 0::numeric THEN sum(shared_url_order.countSales) / sum(social_post.shares)
        ELSE NULL::numeric
      END AS "purchaseRate",
      MAX(tracking_event_history.lightbox_load) AS "clientSales", -- confirmed sales, for now it is the number of times the confirmation page was fired.
      MAX(tracking_event_history.lightbox_clickone_cta) AS "offerClicks", -- First step lightbox click, number of times a soreto link was clicked - it applies if it is a 2 step lightbox
      MAX(tracking_event_history.interstitial_cta)AS "soretoTraffic" -- Click to buy inside interstitial page
    ...
    LEFT JOIN ( select
      ...
      LEFT JOIN (SELECT created_at::date,
              client_id,
              SUM(CASE type WHEN 'interstitial-cta' THEN 1 ELSE 0 END) AS "interstitial_cta",
              SUM(CASE type WHEN 'lightbox-clickone-cta' THEN 1 ELSE 0 END) AS "lightbox_clickone_cta",
              SUM(CASE type WHEN 'lightbox-load' THEN 1 ELSE 0 END) AS "lightbox_load"
            FROM reverb.tracking_event_history
            GROUP BY created_at::date, client_id) tracking_event_history
          ON shared_url.client_id = tracking_event_history.client_id AND tracking_event_history.created_at = dates.date
