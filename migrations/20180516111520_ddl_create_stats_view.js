export function up(knex) {
  ...
  -- Statistics Campaign Version Level Daily
  CREATE OR REPLACE VIEW reverb.agg_campaign_version_stats_daily_js AS
   SELECT
       dates.date,
      client.name AS "clientName",
      client._id AS "clientId",
      COALESCE(campaign._id, 'No Campaign') as "campaignId",
      campaign.description as "campaignName",
      COALESCE(campaign_version._id, 'No Version') as "campaignVersionId",    
     ...
     FROM reverb.shared_url shared_url
       ...
       LEFT OUTER JOIN reverb.client client ON shared_url.client_id = client._id
       LEFT OUTER JOIN reverb.campaign campaign ON client._id = campaign.client_id and shared_url.campaign_id = campaign._id
       LEFT OUTER JOIN reverb.campaign_version campaign_version ON campaign._id = campaign_version.campaign_id and shared_url.campaign_version_id = campaign_version._id
       LEFT OUTER JOIN ( SELECT tracking_event_history_1.created_at::date AS created_at,
            ...
            GROUP BY
                  (tracking_event_history_1.created_at::date),
                  tracking_event_history_1.client_id,
                  tracking_event_history_1.campaign_id,
                  tracking_event_history_1.campaign_version_id
          ...
          ON 
              ...
              AND (shared_url.campaign_id = tracking_event_history.campaign_id OR shared_url.campaign_id is null)
              AND (shared_url.campaign_version_id = tracking_event_history.campaign_version_id OR shared_url.campaign_version_id is null)
              AND tracking_event_history.created_at = dates.date
