...
export function up(knex) {
  var query = `
          DROP FUNCTION reverb.client_stats_per_channel_by_period_js(date,date,text);
          DROP FUNCTION reverb.total_client_stats_by_period_js(date,date,text); 
          
          CREATE OR REPLACE FUNCTION reverb.client_stats_per_channel_by_period_js(date,date,text) 
            RETURNS TABLE("socialPlatform" text, "shares" numeric, "clicks" numeric, "soretoSales" numeric)
              ...
              CREATE OR REPLACE FUNCTION reverb.total_client_stats_by_period_js(date,date,text) 
                RETURNS TABLE(shares numeric, "countSoretoSales" numeric)
                  AS $$    
                      SELECT
