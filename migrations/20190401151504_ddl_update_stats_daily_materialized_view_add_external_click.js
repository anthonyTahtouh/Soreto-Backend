...
exports.up = function(knex) {
  ...
  let script = `
    ...
    SELECT
        ...
        (SELECT EXISTS (SELECT * FROM REVERB.assoc_affiliate_merchant_client WHERE client_id = client._id)) as "externalMerchantHolderConfigured",
        ...
        max(ECU.clicks) AS "clicksUntrackedExternal",
    ...
    JOIN 
        ...
        ON 
        dates.date >= '2018-01-01' and dates.date <= (NOW() + INTERVAL '1 day')
    ...
    ORDER BY 
        dates.date, client.name
    
    WITH NO DATA;

    CREATE UNIQUE INDEX index_agg_campaign_version_stats_daily_js 
    ON 
        reverb.agg_campaign_version_stats_daily_js (date, "clientId", "campaignId", "campaignVersionId");
    `;
