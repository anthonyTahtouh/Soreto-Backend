exports.up = function(knex) {
  ...
  let script = `
      ...
      DROP MATERIALIZED VIEW IF EXISTS reverb.agg_campaign_version_stats_daily_legacy_js;
  
      CREATE MATERIALIZED VIEW reverb.agg_campaign_version_stats_daily_legacy_js
      ...
      ON
      dates.date >= '2018-01-01' and dates.date <= cast(date_trunc('week', (NOW() - INTERVAL '61 day')) as date)
      ...
      --- bring us just one line with full (min and max) period of connection
      LEFT JOIN 
          (
            select 
                assoc1.client_id as client_id
                ,min(assoc1.connected_at) as connected_at
                ,CASE WHEN MAX(COALESCE(assoc1.disconnected_at, '3000-01-01')) = '3000-01-01' THEN NULL ELSE MAX(assoc1.disconnected_at) END as disconnected_at
                ,MAX(assoc1.report_click_source) as report_click_source			
                ,MAX(assoc1.report_order_source) as report_order_source
            from reverb.assoc_affiliate_merchant_client assoc1 
              
              group by assoc1.client_id
              order by assoc1.client_id
          )assoc
      ...
      CREATE UNIQUE INDEX index_agg_campaign_version_stats_daily_legacy_js
      ON
          reverb.agg_campaign_version_stats_daily_legacy_js (date, "clientId", "campaignId", "campaignVersionId");
